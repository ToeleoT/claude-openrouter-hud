...
export async function fetchGeneration(id, apiKey = getAuthToken()) {
  if (!apiKey) {
    return null;
  }

  try {
    const json = await getJson(`/generation?id=${encodeURIComponent(id)}`, apiKey);
...
export async function fetchCredits(apiKey = getAuthToken()) {
  if (!apiKey) {
    return null;
  }

  try {
    const json = await getJson('/credits', apiKey);
    const data = json.data;
