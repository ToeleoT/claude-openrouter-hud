import type { CreditsSnapshot, HudLineInput } from './types.js';
...
function bracket(segment: string): string {
  return `[${segment}]`;
}
...
function levelColor(level: CreditsSnapshot['level']): string {
  if (level === 'critical') {
    return COLORS.red;
  }
  if (level === 'warning') {
...
export function buildHudLine(input: HudLineInput): string {
